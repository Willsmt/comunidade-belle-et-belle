...
export async function uploadFotoJornada(
  arquivo: File,
  clienteId: string,
): Promise<string> {
  ...
  const chave = `jornada-desafio/${clienteId}/${randomUUID()}.webp`;
...
export async function deletarFotoJornada(chave: string): Promise<void> {
