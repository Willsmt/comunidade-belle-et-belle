...
describe("uploadFotoPerfil", () => {
  ...
  it("valida antes de comprimir e envia pro bucket com chave prefixada por perfis-cliente/userId", async () => {
    mockSend.mockResolvedValue({});

    const chave = await uploadFotoPerfil(buildArquivo(), "cliente-1");

    expect(mockComprimirImagem).toHaveBeenCalled();
    expect(mockSend).toHaveBeenCalledTimes(1);
    expect(chave).toMatch(/^perfis-cliente\/cliente-1\/.+\.webp$/);
  });

  it("rejeita arquivo inválido sem chamar o R2", async () => {
    await expect(
      uploadFotoPerfil(buildArquivo({ type: "text/plain" }), "cliente-1"),
...
describe("gerarUrlAssinada", () => {
  ...
  it("gera a signed URL com expiração de 5 minutos", async () => {
    const url = await gerarUrlAssinada("perfis-cliente/cliente-1/foto.webp");
...
describe("deletarFotoPerfil", () => {
  ...
  it("envia o comando de delete pro bucket com a chave certa", async () => {
    ...
    await deletarFotoPerfil("perfis-cliente/cliente-1/foto.webp");
